...
![image-20210302160620511](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210302160620511.png)

注释：

第一层：

connectors：连接（比如JDBC）

第二层：

connection  pool：数据库连接池

management services：服务管理

SQL   interface：sql语句（curd）

Parser：解析器

Optimizer：优化器（mysql会进行二次优化，如阿里就使用自己的）
...
![image-20210304030017900](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210304030017900.png)

```bash
show variables like 'default_storage_engine%';	查看当前数据库正在使用的存储引擎。
```

![image-20210304030125473](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210304030125473.png)

**InnoDB和MyISAM对比**

![image-20210304030202977](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210304030202977.png)

## 三、SQL性能下降原因

- 查询语句写的差。
- 索引（单值，复合）失效：索引建了，但是没有用上。
- 关联 查询太多`join`（设计缺陷或者不得已的需求）。
- 服务器调优以及各个参数的设置（缓冲、线程数等）。

## 四、SQL执行顺序

![image-20210304035421859](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210304035421859.png)

![image-20210304034937512](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210304034937512.png)
...
![image-20210304130905003](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210304130905003.png)
...
![image-20210305125033903](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210305125033903.png)
...
![image-20210305142821342](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210305142821342.png)
...
- 如果id为null，说明是其他行的联合查询结果

#### 2、select_type

数据读取操作的操作类型，主要是用于区别，普通查询、联合查询、子查询等的复杂查询。
...
**从最好到最差依次是（常用）：**`system`>`const`>`eq_ref`>`ref`>`range`>`index`>`ALL`。除了`ALL`没有用到索引，其他级别都用到索引了。

一般来说，得保证查询至少达到`range`级别，最好达到`ref`。

- `system`：表只有一行记录（等于系统表），这是`const`类型的特例，平时不会出现，这个也可以忽略不计。
- `const`：表示通过索引一次就找到了，`const`用于比较`primary key`或者`unique`索引。因为只匹配一行数据，所以很快。如将主键置于`where`列表中，MySQL就能将该查询转化为一个常量。或者唯一的值建立索引。
...
计算公式：https://blog.csdn.net/lzxlfly/article/details/100129240

#### 7、ref

显示索引的哪一列被使用了，如果可能的话，是一个常数。哪些列或常量被用于查找索引列上的值。

分为三个字段（test.t1.name），表示test库的t1表的name字段被使用。（const表示常量）
...
包含不适合在其他列中显示但十分重要的额外信息（前三个）。

**最左前缀原则**：当b+树的数据项是复合的数据结构，比如(name,age,sex)的时候，b+数是按照从左到右的顺序来建立搜索树的，比如当(张三,20,F)这样的数据来检索的时候，b+树会优先比较name来确定下一步的所搜方向，如果name相同再依次比较age和sex，最后得到检索的数据；但当(20,F)这样的没有name的数据来的时候，b+树就不知道下一步该查哪个节点，因为建立搜索树的时候name就是第一个比较因子，必须要先根据name来搜索才能知道下一步去哪里查询。比如当(张三,F)这样的数据来检索时，b+树可以用name来指定搜索方向，但下一个字段age的缺失，所以只能把名字等于张三的数据都找到，然后再匹配性别是F的数据了， 这个是非常重要的性质，即索引的最左匹配特性。
...
- `impossible where`：`WHERE`子句的值总是false，不能用来获取任何元组。(不可能同一个字段同时查到两个值)



![image-20210306050103775](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210306050103775.png)

执行顺序如下：

![image-20210306050127791](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210306050127791.png)

![image-20210306050248864](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210306050248864.png)
...
![image-20210307001401843](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210307001401843.png)
...
![image-20210307003145650](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210307003145650.png)
...
![image-20210307003310059](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210307003310059.png)

创建符合索引`idx_article_ccv`之后，虽然解决了全表扫描的问题，但是在`order by`排序的时候没有用到索引，MySQL居然还是用的`Using filesort`，为什么？
...
![image-20210307003819744](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210307003819744.png)

降序索引（8.0）：https://www.cnblogs.com/wy123/p/9512561.html
...
![image-20210307004439003](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210307004439003.png)

7、当前索引是`idx_article_cv`，来看一下SQL执行计划。

![image-20210307004947364](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210307004947364.png)
...
![image-20210308141708210](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308141708210.png)

`book`和`class`两张表都是没有使用索引，全表扫描，那么如果进行优化，索引是创建在`book`表还是创建在`class`表呢？
...
![image-20210308142137254](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308142137254.png)
...
![image-20210308142606448](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308142606448.png)

此时type类型为ref。

分析：左连接的情况下，左表不管是否存在索引都会做全表扫描，因此，为了执行效率，要在右表创建索引。

结论：**左连接将索引创建在右表上更合适，右连接将索引创建在左表上更合适。**
...
![image-20210308151910875](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308151910875.png)
...
![image-20210308152003573](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308152003573.png)
...
![image-20210308160115052](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308160115052.png)

索引没用上，ALL全表扫描

![image-20210308155906004](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308155906004.png)

用到了idx_staffs_name_age_pos索引中的name字段，pos字段索引失效（否则key_len>74）

![image-20210308160012914](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308160012914.png)
...
+----+-------+-----+---------+---------------------+
|  1 | July |  18 | manager | 2020-08-03 08:30:39 |
+----+-------+-----+---------+---------------------+
...
以上两条SQL的执行结果都是一样的，查看执行效率

![image-20210308161112421](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308161112421.png)

![image-20210308161142833](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308161142833.png)
...
![image-20210308161525663](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308161525663.png)

![image-20210308162104735](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308162104735.png)
...
![image-20210308195356683](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308195356683.png)

![image-20210308195439877](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210308195439877.png)
...
![image-20210309015233037](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309015233037.png)

mysql8.0之后的改变，！=   可以表示为范围。 
...
![image-20210309020029325](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309020029325.png)

![image-20210309020110592](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309020110592.png)

原因：建表的时候设置name不能为null。

关于是否完全不能用null：https://juejin.cn/post/6844903921450745863
...
![image-20210309023556535](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309023556535.png)

![image-20210309023620581](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309023620581.png)
...
![image-20210309024510383](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309024510383.png)

![image-20210309024654720](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309024654720.png)
...
![image-20210309024854812](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309024854812.png)

**口诀：覆盖索引保两边。**

解决模糊查询问题：https://blog.csdn.net/qq_42539533/article/details/93137526

### 9.9字符要加单引号

```bash
/* 使用到了覆盖索引 */
EXPLAIN SELECT `id`, `name` FROM `staffs` WHERE `name` = 'JUly';
...
![image-20210309025701441](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309025701441.png)
...
![image-20210309030057160](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309030057160.png)

```bash
/* 字段不在索引上，索引无效 */
EXPLAIN SELECT * FROM `staffs` WHERE `add_time` = '2020-2-2' or add_time = '2000-2-2';
```

![image-20210309030355571](C:\Users\Z\AppData\Roaming\Typora\typora-user-images\image-20210309030355571.png)
...
| where a = 3 and b like '%kk%' and c = 4                 | 只用到a  （只要%在左边，索引就会失效）     |
...
https://blog.csdn.net/a_hui_tai_lang/article/details/81146635
...
- 查看慢查询日志是否开启：`SHOW VARIABLES LIKE '%slow_query_log%';`。（MySQL8 默认开启）
...
| Opening tables                 | 0.000046 | 0.000000 |   0.000000 |         NULL |          NULL |
...
| Creating tmp table             | 0.001418 | 0.000000 |   0.000000 |         NULL |          NULL |
| executing                      | 0.657946 | 0.625000 |   0.046875 |         NULL |          NULL |
...
| removing tmp table             | 0.000011 | 0.000000 |   0.000000 |         NULL |          NULL |
| waiting for handler commit     | 0.000004 | 0.000000 |   0.000000 |         NULL |          NULL |
...
**全局查询日志**：用于保存所有的sql执行记录，该功能主要用于测试环境，**在生产环境中永远不要开启该功能**。

https://www.cnblogs.com/FondWang/p/12196779.html
